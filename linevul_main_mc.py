...
from linevul_model import Model, compute_adjustment
import pandas as pd
# metrics
...
def test(args, model, tokenizer, test_dataset, cls_threshold=0.5):
    ...
    # y_preds = logits[:, 1] > best_threshold
    # y_preds = np.argmax(logits,1)
    # cls_threshold = 1/len(args.language)
    ...
    # best_threshold, best_f1, best_recall, best_precision = get_best_f1(y_trues==2, logits[:, 2])
    # # print all best
    # print("best_threshold for C: ", best_threshold)
    # print("best_f1 for C: ", best_f1)
    # print("best_recall: ", best_recall)
    # print("best_precision: ", best_precision)
    ...
    table = PrettyTable(['Language','AP','F1','Precision', 'Recall'])
    lans_performance = zip(args.language, ap[1:], f1[1:], precision[1:], recall[1:])
    for lan, ap, f1, precision, recall in lans_performance:
        table.add_row([lan, ap, f1, precision, recall])
...
def main():
    ...
    config.num_labels = 1
    config.num_class = len(args.language) + 1
