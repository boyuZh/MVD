...
from linevul_model import ModelML
import pandas as pd
# metrics
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            loss, logits = model(lan, input_ids=inputs_ids, labels=labels)
            if args.n_gpu > 1:
                loss = loss.mean()
...
def generate_result_df(logits, y_trues, y_preds, args):
    if args.debug:
        df = pd.read_csv(args.test_data_file, nrows=1000)
    else:
        df = pd.read_csv(args.test_data_file)

    if 'language' in df.columns and args.language is not None:
        df = df[df['language'].isin(args.language)]
    df.dropna(subset=['target'], inplace=True)
    df.dropna(subset=['processed_func'], inplace=True)
    all_num_lines = []
...
def main():
    ...
    model = ModelML(model, config, tokenizer, args)
