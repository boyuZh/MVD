...
do 
  if [ $integer -eq 1 ] && [ $train_option -eq 1 ]; then
    if [ -e "./saved_models/ourMC/seed${seed}/checkpoint-best-f1/model.bin" ]; then
...
#SBATCH --mem=16GB
#SBATCH --gres=gpu:1
#SBATCH --err="_logs_linevul/bigvul_results_mc_seed${seed}.err"
#SBATCH --output="_logs_linevul/bigvul_results_mc_seed${seed}.out"
...
python linevul_main_mc.py \
  --output_dir=./saved_models/ourMC/seed${seed} \
  --model_type=roberta \
