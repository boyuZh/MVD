...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        for i in tqdm(range(len(funcs))):
            self.examples.append(convert_examples_to_features(funcs[i], labels[i], commits_url[i], tokenizer, args))
        if file_type == "train":
    ...
    def __getitem__(self, i):
        if self.phase != "test":
            ...
            return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label), self.examples[i].func, self.examples[i].commits_url


def convert_examples_to_features(func, label, commits_url, tokenizer, args):
    if args.use_word_level_tokenizer:
        ...
        source_ids = encoded
        source_tokens = []
        return InputFeatures(func, source_tokens, source_ids, label, commits_url)
    # source
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    for batch in tqdm(test_dataloader):
        inputs_ids, labels, func, commits_url = batch
