...
from transformers import (WEIGHTS_NAME, get_linear_schedule_with_warmup, get_cosine_schedule_with_warmup,
                          RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer)
from tqdm import tqdm
from prettytable import PrettyTable
import multiprocessing
from linevul_model import Model, compute_adjustment, DistillationLoss
import pandas as pd
# metrics
from sklearn.metrics import accuracy_score, recall_score, precision_score, f1_score, average_precision_score, precision_recall_curve
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, df, phase):
        self.phase = phase
        self.examples = []
        # if file_type == "train":
        ...
        # if args include attributes "language"
        # if 'language' in df.columns and args.language:
        #     df = df[df['language']==args.language]

        df.dropna(subset=['target'], inplace=True)
        df.dropna(subset=['processed_func'], inplace=True)
        # df.dropna(subset=['processed_func', 'func_after'], inplace=True)
        # df.dropna(subset=['processed_func', 'func_after'], inplace=True)
        if 'codeLink' in df.columns:
            df['commit_url'] = df['codeLink']
        
        # select the rows whose language is in args.language
        if phase == "train":
            df = df[df['language'].isin(args.trainable_language)]
        else:
        ...
        # translate different languages into numbers using dict
        language_dict = {}
        for i, language in enumerate(args.language):
            language_dict[language] = i+1
        
        print("language_dict: ", language_dict)

        df["target"] = df["target"].astype(int)
        labels = df["target"].tolist()
        commits_url = df["commit_url"].tolist()

        self.df = df

        # change label according to the language if it was 1 otherwise keep 0
        for i in range(len(labels)):
            if labels[i] > 0:
                labels[i] = language_dict[languages[i]]
            else:
                labels[i] = 0

        for i in tqdm(range(len(funcs))):
            self.examples.append(convert_examples_to_features(funcs[i], labels[i], commits_url[i], languages[i], tokenizer, args))
        if phase == "train":
    ...
    def __getitem__(self, i):
        if self.phase != "test":
            return self.examples[i].lan, torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label)
        else:
            return self.examples[i].lan, torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label), self.examples[i].func, self.examples[i].commits_url


def convert_examples_to_features(func, label, commits_url, lan, tokenizer, args):
    if args.use_word_level_tokenizer:
        ...
        source_ids = encoded
        source_tokens = []
        return InputFeatures(func, source_tokens, source_ids, lan, label, commits_url)
    # source
...
def get_best_f1(labels, scores):
    ...
    # print("outlength", len(precision), len(recall), len(thresholds))
    # 计算F分数
    ...
    return thresholdOpt, fscoreOpt, recallOpt, precisionOpt

    
def train(args, train_dataset, model, teacher_model, tokenizer, eval_dataset):
    """ Train the model """
    # build dataloader
    # train_sampler = RandomSampler(train_dataset)
    # train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, num_workers=0)
    disloss = DistillationLoss()
    train_dataloader = DataLoader(train_dataset, batch_size=args.train_batch_size, num_workers=4, shuffle=True)
    if args.logits_adjust:
        args.logit_adjustments = compute_adjustment(train_dataloader, 1.0, args)
        model.set_adjust_logits(args.logit_adjustments, args.post_adjust)
    ...
    # Freeze the parameters of the entire encoder layer
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            lan, inputs_ids, labels = batch
            inputs_ids, labels = inputs_ids.to(args.device), labels.to(args.device)
            # () = [x.to(args.device) for x in batch]
            
            celoss, logits = model(lan, input_ids=inputs_ids, labels=labels, require_logits=True)
            with torch.no_grad():
                teacher_logits = teacher_model(lan, input_ids=inputs_ids, require_logits=True)
            loss_ds = disloss(teacher_logits, logits[:, :args.n_old_class+1])   # +1 means to add the normal class
            if args.n_gpu > 1:
                celoss = celoss.mean()
                loss_ds = loss_ds.mean()
            if args.gradient_accumulation_steps > 1:
                celoss = celoss / args.gradient_accumulation_steps
                loss_ds = loss_ds / args.gradient_accumulation_steps
            ...
            bar.set_description("epoch {} loss {}, celoss {}, dsloss {}".format(idx, avg_loss, celoss.item(), loss_ds.item()))
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            lm_loss, logit = model(lan, input_ids=inputs_ids, labels=labels)
    ...
    # y_pred for the max score
    # y_preds = np.argmax(logits,1)
    # set y_preds to position index if the max score is greater than 0.5, otherwise set as 0
    y_preds = np.where(np.max(logits,1)>best_threshold, np.argmax(logits,1), 0)
    
    print(logits.shape, y_trues.shape)
    recall = recall_score(y_trues, y_preds, average=None)
    precision = precision_score(y_trues, y_preds, average=None)   
    f1 = f1_score(y_trues, y_preds, average='micro')  
    ap = average_precision_score(y_trues, logits, average=None)  
    ...
    result = {
        ...
        "eval_loss": float(eval_loss) / float(nb_eval_steps),
        "eval_cm":cm
    }

    logger.info("***** Eval results *****")
    for key in sorted(result.keys()):
        logger.info("  %s = %s", key, str(np.round(result[key],4)))

    return result

def test(args, model, tokenizer, test_dataset, cls_threshold=0.5):
    # build dataloader
    ...
    for batch in tqdm(test_dataloader):
        lan, inputs_ids, labels, func, commits_url = batch
        inputs_ids, labels = inputs_ids.to(args.device), labels.to(args.device)
        # (inputs_ids, labels, func) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            try:
                lm_loss, logit = model(lan, input_ids=inputs_ids, labels=labels)
            except:
                print(inputs_ids.dtype, labels.dtype)
                exit()
    ...
    # y_preds = logits[:, 1] > best_threshold
    # y_preds = np.argmax(logits,1)
    y_preds = np.where(np.max(logits,1)>cls_threshold, np.argmax(logits,1), 0)
    # seek for best F1

    for i, lan in enumerate(args.language):
        # select the results of the language
        lan_index = np.where(langs==lan)[0]
        # y_trues_lan = y_trues[lan_index] == i+1
        y_trues_lan = y_trues[lan_index] != 0
        # logits_lan = logits[lan_index][:, i+1]
        logits_lan = 1 - logits[lan_index][:, 0]
        # y_preds_lan = y_preds[lan_index] == i+1
        y_preds_lan = logits[lan_index][:, 0] < cls_threshold

        best_threshold, best_f1, best_recall, best_precision = get_best_f1(y_trues_lan, logits_lan)
        # print all best
        print(f"best_threshold for {lan}: ", best_threshold)
        print(f"best_f1 for {lan}: ", best_f1)
        print("best_recall: ", best_recall)
        print("best_precision: ", best_precision)

        acc = accuracy_score(y_trues_lan, y_preds_lan)
        recall = recall_score(y_trues_lan, y_preds_lan, average=None)
        precision = precision_score(y_trues_lan, y_preds_lan, average=None)
        f1 = f1_score(y_trues_lan, y_preds_lan, average=None)
        ap = average_precision_score(y_trues_lan, logits_lan, average=None)
        cm = confusion_matrix(y_trues_lan, y_preds_lan)
        mcc = matthews_corrcoef(y_trues_lan, y_preds_lan)
        result = {
            f"{lan}_accuracy": float(acc),
            f"{lan}_ap": ap,
            f"{lan}_recall": recall,
            f"{lan}_precision": precision,
            f"{lan}_f1": f1,
            f"{lan}_threshold":best_threshold,
            f"{lan}_mcc": mcc,
            f"{lan}_cm": cm
        }
        print (f"***** {lan} Test results *****")
        print(result)

    acc = accuracy_score(y_trues, y_preds)
    recall = recall_score(y_trues, y_preds, average=None)
    precision = precision_score(y_trues, y_preds, average=None)
    f1 = f1_score(y_trues, y_preds, average=None)
    ap = average_precision_score(y_trues, logits, average=None)
    cm = confusion_matrix(y_trues, y_preds)
    ...
    table = PrettyTable(['Language','AP','F1','Precision', 'Recall', 'MCC'])
    lans_performance = zip(args.language, ap[1:], f1[1:], precision[1:], recall[1:], mcc[1:])
    for lan, ap, f1, precision, recall, mcc in lans_performance:
        table.add_row([lan, ap, f1, precision, recall, mcc])
    # table.add_row(['1','server01','服务器01','172.16.0.1'])
    print(table)

    # output func, logits and y_trues in a csv file
    raw_preds = pd.DataFrame()
    funcs = [item for sublist in funcs for item in sublist]
    commits_urls = [item for sublist in commits_urls for item in sublist]
    raw_preds["func"] = funcs
    raw_preds["scores"] = logits[:, 1]
    raw_preds["preds"] = y_preds
    raw_preds["y_trues"] = y_trues
    raw_preds["commits_urls"] = commits_urls
    raw_preds.to_csv("./results/raw_preds.csv", index=False)
    ...
    result_df = generate_result_df(logits, y_trues, y_preds, test_dataset)
...
def generate_result_df(logits, y_trues, y_preds, dataset):
    df = dataset.df
    ...
    print(len(logits) , len(y_trues) , len(y_preds) , len(all_num_flaw_lines))
...
def main():
    ...
    ## parameters
    parser.add_argument("--data_file", default=None, type=str, required=False,
    ...
    parser.add_argument('--n_old_class', type=int, default=1,
                        help="num of class")
    parser.add_argument('--language', type=str, default=None,
                        help="language to select")
    parser.add_argument('--resume', type=str, default=None,
                        help="Whether training from specified model.")
    parser.add_argument("--freeze_encoder", action='store_true',
                        help="Whether freeze the encoder.")
    parser.add_argument("--logits-adjust", action='store_true',
                        help="Whether adjust the logits. (Long-Tail Learning via Logit Adjustment)")
    parser.add_argument("--post-adjust", action='store_true',
                        help="Whether post adjust the logits. (Long-Tail Learning via Logit Adjustment)")
    ...
    if args.language is None:
        exit("Please specify language!")
    else:
        if isinstance(args.language, str) and ',' in args.language:
            args.language = args.language.split(',')
        else:
            args.language = [args.language]
    
    assert args.n_old_class < len(args.language)
    args.trainable_language = args.language[args.n_old_class:]


    config = RobertaConfig.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)
    config.num_labels = 1
    config.num_class = args.n_old_class + len(args.trainable_language) + 1
    ...
    teacher_config.num_class = config.num_class - 1
    teacher_model = RobertaForSequenceClassification.from_pretrained(args.model_name_or_path, config=teacher_config, ignore_mismatched_sizes=True)
    ...
    if args.resume is not None:
        ...
        for k in state_dict.keys():
            if k in model_dict: # 排除fc层
                if "out_proj.weight" in k:
                    model_dict[k][:teacher_config.num_class] = state_dict[k]
    ...
    if args.debug:
        df = df.sample(n=10000, random_state=args.seed)
        args.epochs = 1

    # Split the DataFrame into train and remaining
    train_df, remaining_df = train_test_split(df, test_size=0.2, random_state=args.seed)

    # Split the remaining DataFrame into validation and test
    val_df, test_df = train_test_split(remaining_df, test_size=0.5, random_state=args.seed)

    # Training
    if args.do_train:
        train_dataset = TextDataset(tokenizer, args, train_df, 'train')
        eval_dataset = TextDataset(tokenizer, args, val_df, 'eval')
        train(args, train_dataset, model, teacher_model, tokenizer, eval_dataset)
    # Evaluation
    results = {}
    if args.do_test:
        ...
        model.to(args.device)
        test_dataset = TextDataset(tokenizer, args, test_df, 'test')
        test(args, model, tokenizer, test_dataset, cls_threshold=0.5)
    return results
