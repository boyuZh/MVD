...
from sklearn.metrics import auc, confusion_matrix, precision_recall_curve, matthews_corrcoef
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, df, phase):
        ...
        # if file_type == "train":
        #     extra_funcs = df[df['target']==1]["func_after"].tolist()
        #     funcs.extend(extra_funcs)
        #     extra_target = [0] * len(extra_funcs)
        #     labels.extend(extra_target)
        #     extra_urls = df[df['target']==1]["commit_url"].tolist()
        #     commits_url.extend(extra_urls)
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    ap = average_precision_score(y_trues, y_preds)  
...
def test(args, model, tokenizer, test_dataset, cls_threshold=0.5):
    ...
    logits = np.concatenate(logits, 0)
    y_trues = np.concatenate(y_trues, 0)
    y_preds = logits[:, 1] > cls_threshold
    ...
    best_threshold, best_f1, best_recall, best_precision = get_best_f1(y_trues, logits[:, 1])
    # print all best
    print(f"best_threshol: ", best_threshold)
    print(f"best_f1 for: ", best_f1)
    print("best_recall: ", best_recall)
    print("best_precision: ", best_precision)
    ...
    print("***** Test results *****")
    for key in sorted(result.keys()):
        print("  %s = %s", key, str(np.round(result[key],4)))
...
def main():
    ...
    #     args.model_name_or_path = "/hpcfs/users/a1232991/.cache/huggingface/hub/models--microsoft--codebert-base/snapshots/3b0952feddeffad0063f274080e3c23d75e7eb39"
    # if args.tokenizer_name == "microsoft/codebert-base":
    #     args.tokenizer_name = "/hpcfs/users/a1232991/.cache/huggingface/hub/models--microsoft--codebert-base/snapshots/3b0952feddeffad0063f274080e3c23d75e7eb39"
